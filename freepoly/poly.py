...
class Poly:
    ...
    def enumerate_poly(self, n):
        ...
        for i in range(2, n + 1):
            ...
            while size > 0:
                ...
                size = size - 1
        return list(free_poly_queue)

    def save_poly(self, free_poly_set, free_poly_queue, poly_n):
        if poly_n is not None:
            isUniquePoly = self.isUniquePoly(free_poly_set, poly_n)
            if isUniquePoly:
                free_poly_set.add(tuple(poly_n))
    ...
    def isUniquePoly(self, free_poly_set, free_poly):
        transformations = self.transformations(free_poly)
        for t in transformations:
            if tuple(t) in free_poly_set:
                return False
        return True
